require "sat_mx/download_request_received_body"
require "nokogiri"

RSpec.describe SatMx::DownloadRequestReceivedBody do
  let(:certificate) do
    OpenSSL::X509::Certificate.new(
      fixture("local_business/2526_mifiel_local_business.cer")
    )
  end

  describe "#generate" do
    it do
      start_date = Time.new(2024, 10, 1)
      end_date = Time.new(2024, 10, 21)
      request_type = :cfdi
      recipient_rfc = "AAA010101AAA"
      requester_rfc = "AAA010101AAA"

      download_request_received_body = SatMx::DownloadRequestReceivedBody.new(
        start_date:,
        end_date:,
        request_type:,
        recipient_rfc:,
        requester_rfc:,
        certificate:
      )

      expect(Nokogiri::XML::Document.parse(download_request_received_body.generate)).to be_same_xml(expected_xml)
    end
  end

  describe "#complement" do
    it "returns the complement value" do
      download_request_received_body = SatMx::DownloadRequestReceivedBody.new(
        start_date: Time.new(2024, 10, 1),
        end_date: Time.new(2024, 10, 21),
        request_type: :cfdi,
        recipient_rfc: "AAA010101AAA",
        requester_rfc: "AAA010101AAA",
        certificate:,
        complement: "nomina12"
      )

      expect(download_request_received_body.complement).to eq("nomina12")
    end

    it "returns nil when not provided" do
      download_request_received_body = SatMx::DownloadRequestReceivedBody.new(
        start_date: Time.new(2024, 10, 1),
        end_date: Time.new(2024, 10, 21),
        request_type: :cfdi,
        recipient_rfc: "AAA010101AAA",
        requester_rfc: "AAA010101AAA",
        certificate:
      )

      expect(download_request_received_body.complement).to be_nil
    end
  end

  describe "#document_status" do
    it "returns the document_status value" do
      download_request_received_body = SatMx::DownloadRequestReceivedBody.new(
        start_date: Time.new(2024, 10, 1),
        end_date: Time.new(2024, 10, 21),
        request_type: :cfdi,
        recipient_rfc: "AAA010101AAA",
        requester_rfc: "AAA010101AAA",
        certificate:,
        document_status: "Cancelado"
      )

      expect(download_request_received_body.document_status).to eq("Cancelado")
    end

    it "returns nil when not provided" do
      download_request_received_body = SatMx::DownloadRequestReceivedBody.new(
        start_date: Time.new(2024, 10, 1),
        end_date: Time.new(2024, 10, 21),
        request_type: :cfdi,
        recipient_rfc: "AAA010101AAA",
        requester_rfc: "AAA010101AAA",
        certificate:
      )

      expect(download_request_received_body.document_status).to be_nil
    end
  end

  describe "with optional parameters" do
    it "includes issuing_rfc when provided" do
      start_date = Time.new(2024, 10, 1)
      end_date = Time.new(2024, 10, 21)
      request_type = :cfdi
      recipient_rfc = "AAA010101AAA"
      requester_rfc = "AAA010101AAA"
      issuing_rfc = "MOCR690424NZ5"

      download_request_received_body = SatMx::DownloadRequestReceivedBody.new(
        start_date:,
        end_date:,
        request_type:,
        recipient_rfc:,
        requester_rfc:,
        certificate:,
        issuing_rfc:
      )

      xml_doc = Nokogiri::XML::Document.parse(download_request_received_body.generate)
      solicitud = xml_doc.at_xpath("//des:solicitud", des: "http://DescargaMasivaTerceros.sat.gob.mx")

      expect(solicitud.attributes["RfcEmisor"].value).to eq(issuing_rfc)
    end

    it "includes complement when provided" do
      start_date = Time.new(2024, 10, 1)
      end_date = Time.new(2024, 10, 21)
      request_type = :cfdi
      recipient_rfc = "AAA010101AAA"
      requester_rfc = "AAA010101AAA"
      complement = "nomina12"

      download_request_received_body = SatMx::DownloadRequestReceivedBody.new(
        start_date:,
        end_date:,
        request_type:,
        recipient_rfc:,
        requester_rfc:,
        certificate:,
        complement:
      )

      xml_doc = Nokogiri::XML::Document.parse(download_request_received_body.generate)
      solicitud = xml_doc.at_xpath("//des:solicitud", des: "http://DescargaMasivaTerceros.sat.gob.mx")

      expect(solicitud.attributes["Complemento"].value).to eq(complement)
    end

    it "includes document_status when provided" do
      start_date = Time.new(2024, 10, 1)
      end_date = Time.new(2024, 10, 21)
      request_type = :cfdi
      recipient_rfc = "AAA010101AAA"
      requester_rfc = "AAA010101AAA"
      document_status = "Cancelado"

      download_request_received_body = SatMx::DownloadRequestReceivedBody.new(
        start_date:,
        end_date:,
        request_type:,
        recipient_rfc:,
        requester_rfc:,
        certificate:,
        document_status:
      )

      xml_doc = Nokogiri::XML::Document.parse(download_request_received_body.generate)
      solicitud = xml_doc.at_xpath("//des:solicitud", des: "http://DescargaMasivaTerceros.sat.gob.mx")

      expect(solicitud.attributes["EstadoComprobante"].value).to eq(document_status)
    end
  end

  let(:expected_xml) do
    Nokogiri::XML::Document.parse(
      <<~XML.strip
        <S11:Envelope xmlns:S11="http://schemas.xmlsoap.org/soap/envelope/" xmlns:des="http://DescargaMasivaTerceros.sat.gob.mx" xmlns:ds="http://www.w3.org/2000/09/xmldsig#">
          <S11:Header/>
          <S11:Body>
            <des:SolicitaDescargaRecibidos>
              <des:solicitud FechaInicial="2024-10-01T00:00:00" FechaFinal="2024-10-21T00:00:00" TipoSolicitud="CFDI" RfcReceptor="AAA010101AAA" RfcSolicitante="AAA010101AAA">
                <Signature xmlns="http://www.w3.org/2000/09/xmldsig#">
                  <SignedInfo>
                    <CanonicalizationMethod Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315"/>
                    <SignatureMethod Algorithm="http://www.w3.org/2000/09/xmldsig#rsa-sha1"/>
                    <Reference URI="">
                      <Transforms>
                        <Transform Algorithm="http://www.w3.org/2000/09/xmldsig#enveloped-signature"/>
                      </Transforms>
                      <DigestMethod Algorithm="http://www.w3.org/2000/09/xmldsig#sha1"/>
                      <DigestValue></DigestValue>
                    </Reference>
                  </SignedInfo>
                  <SignatureValue></SignatureValue>
                  <KeyInfo>
                    <X509Data>
                      <X509IssuerSerial>
                        <X509IssuerName>emailAddress=info@mifiel.com,CN=Mifiel Intermediate,O=Mifiel,C=MX</X509IssuerName>
                        <X509SerialNumber>9510</X509SerialNumber>
                      </X509IssuerSerial>
                      <X509Certificate></X509Certificate>
                    </X509Data>
                  </KeyInfo>
                </Signature>
              </des:solicitud>
            </des:SolicitaDescargaRecibidos>
          </S11:Body>
        </S11:Envelope>
      XML
    )
  end
end
